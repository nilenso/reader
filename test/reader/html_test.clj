...
(deftest new-html-test
  (testing "Given html with no images or css, return unchanged html"
    (let [html "<html><head></head><body></body></html>"]
      (is (= html (h/change-html html h/remove-node-css h/remove-node-images)))))
  ...
  (testing "Given html with images, return html without them"
    (let [html1 "<html><head></head><body><img src='' alt=''></body></html>"
      ...
      (is (= changed-html (h/change-html html1 h/remove-node-images)))
      (is (= changed-html (h/change-html html2 h/remove-node-images)))
      (is (= changed-html (h/change-html html3 h/remove-node-images)))
      (is (= changed-html4 (h/change-html html4 h/remove-node-images)))))
  ...
  (testing "Given html with css styling, return html without"
    (let [html1 "<html><head><style></style></head><body></body></html>"
      ...
      (is (= changed-html (h/change-html html1 h/remove-node-css)))
      (is (= changed-html2 (h/change-html html2 h/remove-node-css)))
      (is (= changed-html (h/change-html html3 h/remove-node-css)))
      (is (= changed-html4 (h/change-html html4 h/remove-node-css)))))
  ...
  (testing "Given any html, remove images, css, and add font styling"
    (let [html1 "<html><head><style></style></head><body></body></html>"
      ...
      (is (= changed-html (h/change-html html1)))
      (is (= changed-html (h/change-html html2))))))
